...
impl FarDesk {
    ...
    ///
    /// A pair exchange can break mid-way — the model errors, or returns
    /// nothing — and the rows already said are durable. Nothing exercised
    /// either path (tinysweeper, #2341).
    fn failing_after(turns: usize, answer: &str) -> Self {
        Self {
    ...
    fn silent_after(turns: usize, answer: &str) -> Self {
        Self {
...
impl HiveReferralRunner for FarDesk {
    async fn refer(&self, desk_id: &str, agent_id: &str, prompt: &str) -> Result<String> {
        ...
        if self.falters_after.is_some_and(|after| ran > after) {
...
/// The cap bounds questions that CROSS, and a teammate is not a crossing.
///
/// `peer_cap` is documented as "how many crossing questions one episode may
/// ask", priced on a question costing a full model turn on another desk — but
/// the check was gated on `ReferralKind::Forward` alone, which a same-desk
/// question also is. So asking two peer desks exhausted the allowance and
/// every later question to a colleague on this seat's OWN desk was refused.
///
/// That is what stopped agent-to-agent DMs happening at all: one live episode
/// spent the cap on two `@#returns` crossings and then refused eight
/// consecutive `@refunds`/`@amendments` questions (tinysweeper, #2341).
#[tokio::test]
async fn a_question_to_a_teammate_does_not_spend_the_peer_desk_allowance() {
    ...
    let (log, outcome) = run(
        hive,
        &[
            // Spends the whole allowance: this one leaves the desk.
            ("planner", "!question #a A peer desk question. @#platform"),
            // Must still be asked. It never leaves the desk, so it costs no
            // far-desk turn and the cap has no claim on it.
    ...
    // `FarDesk` records every turn it is asked to run, and a same-desk pair
    // turn runs through the same port — so count the ones that actually landed
    // on another DESK, which is what the cap is about.
...
///
/// The rows are durable and the asker has something real to carry home, which
/// is better than discarding an answer because a follow-up failed. Both break
/// paths — a failing turn and an empty reply — were written and neither was
/// exercised (tinysweeper, #2341).
#[tokio::test]
async fn a_pair_exchange_that_breaks_keeps_the_rows_it_already_has() {
    ...
    // The third turn errors, so the exchange stops with the question and the
    // answer it did get — not with nothing.
    ...
    // An empty reply is a stop, not a blank row: nothing is journaled for it.
...
/// The asker's next turn is actually handed the exchange it just had.
///
/// This is the whole point of a private line: the pair settles something, and
/// the asker carries the conclusion onto the floor in its own words. The rows
/// live in `dm:<a>+<b>`, which `elsewhere_for` never gathers — so if the
/// continuation prompt does not carry them, the pair agreed and the room
/// watched both seats re-derive it in the open.
///
/// Bounded at the row this exchange opened on, too: the pair key is
/// deterministic, so an unbounded read hands the asker every crossing those
/// two ever had (tinysweeper, #2341).
#[tokio::test]
async fn the_asker_is_handed_the_exchange_it_just_had_and_nothing_older() {
    ...
    // An older exchange between the same two, already in their pair thread
    // before this episode opens. A deterministic key means this is the very
    // same conversation the new crossing will use.
    let pair = super::referral::pair_conversation("planner", "sre");
    for (who, text) in [
    ...
    // The turn `planner` took AFTER its question is the continuation.
