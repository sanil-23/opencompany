...
export function AppShell({
   ...
   * down for *this* render without a reload (`gateSkipped` already feeds both
   * predicates); the durable one is what stops the gate being owed again on the
   * next fresh tab. Only this button writes the durable marker — `leaveGateFor`
   * below still writes the session marker alone, because following a link into
   * Workflows is the founder going to *do* a step, not an answer to it.
   *
   ...
   */
  const [gateIsDismissed, setGateIsDismissed] = useState(() =>
    gateDismissed(scope),
  );
  useEffect(() => {
   ...
   */
  const [gateWaived, setGateWaived] = useState<GateStepId[]>(() =>
  ...
  useEffect(() => {
    ...
    const scopeJustChanged =
      previous === null ||
      previous.connection !== scope.connection ||
    ...
    setGateWaived((previous) => {
      ...
      const same =
        previous.length === stored.length &&
        stored.every((step, i) => previous[i] === step);
      ...
      .then((feed) => {
        ...
        if (toAnnounce.length > 0) {
          ...
          setMentionFeed((current) =>
            current.map((n) =>
              ids.includes(n.id) ? { ...n, readAt: Date.now() } : n,
            ),
          );
          ...
          if (ackNow.length > 0) {
            void client
              .markNotificationsRead(ackNow, requestCompany)
              .catch(() => {
  ...
  useEffect(() => {
    const onVisibilityChange = () => {
      ...
      if (ackNow.length > 0) {
        void client
          .markNotificationsRead(ackNow, scopeRef.current.company)
          .catch(() => {
